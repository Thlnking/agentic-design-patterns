...
虽然单体智能体对于定义明确的问题可能是有效的，但在面对复杂的多领域任务时，其能力往往受到限制。多智能体协作模式通过将系统构建为由不同专门化智能智能体作集合来解决这些限制。这种方法基于任务分解原则，其中高级目标被分解为离散的子问题。然后将每个子问题分配给拥有最适合该任务的特定工具、数据访问或推理能力的智能体。
...
* **顺序交接：** 一个智能体任务并将其输出传递给另一个智能智能体道中的下一步（类似于规划模式，但明确涉及不同的智能体）。
* **并行处理：** 多个智能体处理问题的不同部分，然后它们的结果稍后被组合。
* **辩论和共识：** 多智能体协作，其中具有不同观点和信息来源的智能体讨论以评估选项，最终达成共识或更明智的决策。
* **层次结构：** 管理者智能体根据其工具访问或插件能力动态地将任务委托给工作智能体，并综合其结果。每个智能智能体理相关的工具组，而不是单个智能体处智能体。
...
多智能体（见图 1）从根本上包括智能智能体责的界定、智能体通过其交换信息的通信渠道的建立，以及指导其协作努力的任务流或交互协议的制定。
...
**1. 单智能体：** 在最基本的层面上，"单智能体"在没有与其他实体直接交互或通信的情况下自主运行。虽然此模型易于实现和管理，但其能力固有地受到单个智能体和资源的限制。它适用于可分解为独立子问题的任务，每个子问题都可由单个自给自足的智能智能体
...
**3. 监督者：** 在"监督者"模型中，专用智能体（"监督者"）监督和协调一组下属智能体动。监督者充当通信、任务分配和冲突解决的中心枢纽。这种层次结构提供了清晰的权限线，可以简化管理和控制。然而，它引入了单点故障（监督者），如果监督者被大量下属或复杂任务压倒，可能会成为瓶颈。
...
总之，为多智能体选择相互关系和通信模型是关键的设计决策。每个模型提供不同的优势和劣势，最佳选择取决于诸如任务复杂性、智能体数量、期望的自主程度、对稳健性的需求以及可接受的通信开销等因素。多智能智能体来进展可能会继续探索和完善这些模型，以及开发协作智能的新范式。
...
相应地定义了两个任务：一个用于研究趋势，另一个用于撰写博客文章，写作任务依赖于研究任务的输出。然后将这些智能体务组装成一个团队，指定顺序流程，其中任务按顺序执行。团队使用智能体、任务和语言模型（特别是"gemini-2.0-flash"模型）初始化。主函数使用 kickoff() 方法执行此团队，编排智能智能体作以产生所需的输出。最后，代码打印团队执行的最终结果，即生成的博客文章。
...
**为什么：** 多智能体协作模式通过创建多个协作智能体统提供了标准化解决方案。复杂问题被分解为更小的更易于管理的子问题。然后将每个子问题分配给具有解决它所需的精确工具和能力的专门智能体。这些智能智能体的通信协议和交互模型（如顺序交接、并行工作流或层次化委托）协同工作。这种智能体化智能体法创造了协同效应，使团队能够实现任何单个智能体都无法智能体。
